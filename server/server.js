...
const attendanceSchema = new mongoose.Schema({
    ...
    scanHistory: [{
        ...
        source: {
            type: String,
            enum: ["ADMIN_EMPLOYEE_QR", "EMPLOYEE_ADMIN_QR", "WHATSAPP_IMPORT", "ADMIN_EDIT"],
...
const applyAttendanceScan = async({
    ...
    if (scanType === "ENTRY") {
        ...
        attendance.lastOutTime = scannedAt;
    ...
    const historyEntry = {
        ...
        scannedAt,
        source,
        scannedBy: scannedBy || identity.username || identity.email || employeeUserId,
        qrVersion: qrVersion || "",
        whatsappStatus: notifyWhatsApp && isWhatsAppIntegrationConfigured() ? "pending" : "skipped",
    ...
    let whatsapp = {
        sent: false,
        status: "skipped",
        provider: "disabled-for-this-scan",
        error: "WhatsApp notification was not requested for this scan.",
    };
    ...
    if (notifyWhatsApp) {
        ...
        const lastHistoryEntry = attendance.scanHistory[attendance.scanHistory.length - 1];

        if (lastHistoryEntry) {
            lastHistoryEntry.whatsappStatus = whatsapp.status || (whatsapp.sent ? "sent" : "failed");
...
app.post("/api/my/scan-admin-qr", verifyToken, async(req, res) => {
    try {
        ...
        const result = await applyAttendanceScan({
            ...
            scannedBy: employee.username || employee.email || String(employee._id),
            qrVersion,
            notifyWhatsApp: true,
        });
        ...
        await createAuditLog(req, {
            ...
            description: `${result.message} by employee self scan for ${result.identity.username || result.identity.name || result.identity.email || result.employeeUserId}`,
...
const recalculateAttendanceTotals = (attendance) => {
    ...
    if (attendance.breakOutTime && attendance.breakInTime) {
        totalBreakSeconds = Math.floor((attendance.breakInTime - attendance.breakOutTime) / 1000);
    ...
    if (attendance.entryTime && attendance.lastOutTime) {
        totalWorkSeconds = Math.floor((attendance.lastOutTime - attendance.entryTime) / 1000) - totalBreakSeconds;
...
app.post("/api/attendance/import-whatsapp", verifyAdmin, async(req, res) => {
    try {
        ...
        for (const line of lines) {
            ...
            if (parsed.scanType === "ENTRY") {
                attendance.entryTime = scanDateTime;
            } else if (parsed.scanType === "BREAK_OUT") {
                attendance.breakOutTime = scanDateTime;
            } else if (parsed.scanType === "BREAK_IN") {
                attendance.breakInTime = scanDateTime;
            } else if (parsed.scanType === "OUT") {
                attendance.lastOutTime = scanDateTime;
            } else if (parsed.scanType === "OUT_WITHOUT_BREAK") {
                attendance.lastOutTime = scanDateTime;
                attendance.breakOutTime = null;
                attendance.breakInTime = null;
            }
