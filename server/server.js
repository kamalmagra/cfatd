...
const parseWhatsAppAttendanceLine = (line = "", fallbackDate = "") => {
    ...
    match = rawLine.match(/^(.+?)\s+(entry|in|check\s*in|break\s*out|breakout|break\s*in|breakin|last\s*out|check\s*out|checkout|out|finish|end)\s+(\d{1,2}:\d{2})(?:\s+(\d{1,2}[\/\-.]\d{1,2}(?:[\/\-.]\d{2,4})?|\d{4}-\d{2}-\d{2}))?$/i);
    ...
    match = rawLine.match(/^(.+?)\s+(\d{1,2}:\d{2})\s+(entry|in|check\s*in|break\s*out|breakout|break\s*in|breakin|last\s*out|check\s*out|checkout|out|finish|end)(?:\s+(\d{1,2}[\/\-.]\d{1,2}(?:[\/\-.]\d{2,4})?|\d{4}-\d{2}-\d{2}))?$/i);
